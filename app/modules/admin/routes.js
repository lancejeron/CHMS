...
function fregcount(req, res, next){
  ...
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'verified' OR strStatus= 'not verified'", function (err, results, fields) {
      if (err) return res.send(err);
      req.regcount = results;
      return next();
  });
}
function fusercount(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'verified' AND strType='1'" , function (err, results, fields) {
      if (err) return res.send(err);
...
function fnvercount(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'not verified'", function (err, results, fields) {
      if (err) return res.send(err);
...
function funregcount(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'unregistered'", function (err, results, fields) {
      if (err) return res.send(err);
...
function frejcount(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'rejected'", function (err, results, fields) {
      if (err) return res.send(err);
...
function fbancount(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT COUNT(intID) AS CNT FROM dbcfm.tbluser WHERE strStatus= 'banned'", function (err, results, fields) {
      if (err) return res.send(err);
...
function fstudunreg(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM dbcfm.tbluser WHERE strStatus= 'unregistered'";
...
function fstudreject(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM dbcfm.tbluser WHERE strStatus= 'rejected'";
  db.query(sql, function (err, results, fields) {
      if (err) return res.send(err);
      var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [], status = [];
      if (!results[0])
...
function freguser(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM dbcfm.tbluser WHERE intID= ?";
...
function fverified(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT * FROM tbluser WHERE strStatus= 'verified'", function (err, results, fields) {
      if (err) return res.send(err);
      var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [], status = [];
      if (!results[0])
        console.log('');
...
function fbanned(req, res, next){
  var db = require('../../lib/database')();
  db.query("SELECT * FROM tbluser WHERE strStatus= 'banned'", function (err, results, fields) {
      if (err) return res.send(err);
      var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [], status = [];
      if (!results[0])
        console.log('');
...
//   var db = require('../../lib/database')();
//   db.query("SELECT *, COUNT(*) AS CNT FROM(SELECT * FROM(SELECT * FROM(SELECT * FROM tblitem INNER JOIN tblreport ON intItemID= intReportItemID)AS A INNER JOIN tblcategories ON intItemCat= intCatID)AS B INNER JOIN tbluser ON strItemSNum= intID)AS C LEFT JOIN tbltransaction ON intItemID= intTransItemID WHERE strTransStatus!= 'Finished' OR strTransStatus IS NULL GROUP BY intItemID ORDER BY CNT DESC", function (err, results, fields) {
//       if (err) return res.send(err);
...
//   var db = require('../../lib/database')();
//   db.query("SELECT * FROM(SELECT * FROM(SELECT * FROM(SELECT * FROM tblreport INNER JOIN tblitem ON intItemID= intReportItemID)A INNER JOIN tbluser ON strReportSNum= intID)B INNER JOIN tblcategories ON intItemCat= intCatID)C LEFT JOIN tbltransaction ON intTransItemID= intItemID WHERE strTransStatus!= 'Finished' OR strTransStatus IS NULL ORDER BY intReportID DESC", function (err, results, fields) {
//       if (err) return res.send(err);
//       var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [];
//       if (!results[0])
//         console.log('');
...
function approverender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tbluser SET strStatus= 'verified' WHERE intID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
...
function rejectrender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tbluser SET strStatus= 'rejected' WHERE intID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
      res.redirect('/admin/registration-unregistered/1');
    });
...
function revertrender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tbluser SET strStatus= 'unregistered' WHERE intID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
...
function banverrender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tbluser SET strStatus= 'banned' WHERE intID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
...
//   if(req.valid==2){
//     var db = require('../../lib/database')();
//     var sqlban = "UPDATE tbluser SET strStatus= 'banned' WHERE intID= ?";
...
router.get('/', flog, fregcount, fusercount, fnvercount, funregcount, frejcount, fbancount, render);
...
function fcoffeeunreg(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM dbcfm.tblcoffeeshop WHERE strStatus= 'unregistered'";
  db.query(sql, function (err, results, fields) {
      if (err) return res.send(err);
      var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [], status = [];
      if (!results[0])
...
function coffeeunregrender(req,res){
  if(req.valid==2){
    if(!req.stud[0]){
      if(req.params.page == 1)
        res.render('admin/views/nocoffeeunreg');
      else
        res.render('login/views/noroute');
    }
    else if(req.params.page < 1 || req.params.page > req.lastpage[0])
      ...
      res.render('admin/views/coffeeshop',{studtab: req.stud, pagetab: req.page, curpagetab: req.curpage, prevpagetab: req.prevpage, nextpagetab: req.nextpage, lastpagetab: req.lastpage, statustab: req.status});
  }
  else if(req.valid==1)
    res.render('admin/views/invalidpages/normalonly');
  else
...
router.get('/coffeeshop/:page',flog,fcoffeeunreg,coffeeunregrender)



function fregcoffee(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM tblcoffeeshop WHERE intCID= ?";
  db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
      req.reguser = results;
      return next();
...
function approvecoffeeshoprender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tblcoffeeshop SET strStatus= 'registered' WHERE intCID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
      res.redirect('/admin/coffeeshop/1');
    });
  }
  else if(req.valid==1)
    res.render('admin/views/invalidpages/normalonly');
...
router.get('/coffeeshop/approve/:userid', flog,fregcoffee,approvecoffeeshoprender)

function fcoffeereject(req, res, next){
  var db = require('../../lib/database')();
  var sql = "SELECT * FROM tblcoffeeshop WHERE strStatus= 'rejected'";
  db.query(sql, function (err, results, fields) {
      if (err) return res.send(err);
      var page = 1, pagearr = [1], curpage = [req.params.page], prevpage = [req.params.page - 1], nextpage = [parseInt(req.params.page)+1], lastpage = [], status = [];
      if (!results[0])
...
function coffeerejrender(req,res){
  if(req.valid==2){
    if(!req.stud[0]){
      if(req.params.page == 1)
        res.render('admin/views/nocoffeeshoprej');
      else
        res.render('login/views/noroute');
    }
    else if(req.params.page < 1 || req.params.page > req.lastpage[0])
...
router.get('/coffeeshop-rejected/:page', flog, fcoffeereject, coffeerejrender);


function rejectcoffeerender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tblcoffeeshop SET strStatus= 'rejected' WHERE intCID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
      res.redirect('/admin/coffeeshop/1');
    });
...
router.get('/coffeeshop/reject/:userid', flog, rejectcoffeerender);


function revertcoffeerender(req,res){
  if(req.valid==2){
    var db = require('../../lib/database')();
    var sql = "UPDATE tblcoffeeshop SET strStatus= 'unregistered' WHERE intCID= ?";
    db.query(sql,[req.params.userid], function (err, results, fields) {
      if (err) return res.send(err);
      res.redirect('/admin/coffeeshop/1');
    });
...
router.get('/coffeeshop/revert/:userid', flog, revertcoffeerender);
